...
pub async fn run_interactive_setup(mut config: Config) -> Result<Config> {
    ...
    println!(
        "{}",
        "💀 Let's configure RIP for your security scanning needs. 💀\n"
            .bright_green()
            .bold()
    );
    ...
    let repo_path: String = Input::new()
        .with_prompt(format!(
            "{} {}",
            "🎯".bright_yellow(),
            "Repository directory to scan".bright_cyan().bold()
        ))
    ...
    let env_file: String = Input::new()
        .with_prompt(format!(
            "{} {}",
            "📄".bright_yellow(),
            "Environment file name".bright_cyan().bold()
        ))
    ...
    println!(
        "\n{} {}",
        "🔍".bright_yellow(),
        "Parsing environment file...".bright_cyan()
    );
    ...
    if available_keys.is_empty() {
        println!(
            "{} {}",
            "⚠️".bright_yellow(),
            "No environment variables found or all values are trivial".bright_red()
        );
        println!(
            "{} {}",
            "➡️".bright_green(),
            "Continuing with empty key list...".bright_white()
        );
    } else {
        println!(
            "{} {} {}",
            ...
            "Found".bright_cyan(),
            format!(
                "{} environment variables with non-trivial values:",
                available_keys.len()
            )
            .bright_white()
            .bold()
        ...
        let selected_indices = MultiSelect::new()
            .with_prompt(format!(
                "{} {}",
                "🔑".bright_yellow(),
                "Select environment keys to scan for".bright_cyan().bold()
            ))
        ...
        println!(
            ...
            format!("{} keys for scanning", config.env_keys.len())
                .bright_white()
                .bold()
        );
    }
    ...
    let selected_ext_indices = MultiSelect::new()
        .with_prompt(format!(
            "{} {}",
            "🔧".bright_yellow(),
            "Select file extensions to include in scan"
                .bright_cyan()
                .bold()
        ))
    ...
    let selected_ignore_indices = MultiSelect::new()
        .with_prompt(format!(
            "{} {}",
            "⛔".bright_yellow(),
            "Select patterns to ignore during scan".bright_cyan().bold()
        ))
    ...
    println!(
        "\n{} {}",
        "🌐".bright_yellow(),
        "Web scanning configuration...".bright_cyan().bold()
    );
    config.enable_web_scan = Confirm::new()
        .with_prompt(format!(
            "{} {}",
            "🔗".bright_yellow(),
            "Enable web vulnerability scanning?".bright_cyan().bold()
        ))
        .default(false)
        .interact()?;
    ...
    if config.enable_web_scan {
        let web_url: String = Input::new()
            .with_prompt(format!(
                "{} {}",
                "🌍".bright_yellow(),
                "Public URL for web scanning".bright_cyan().bold()
            ))
            .allow_empty(true)
            .interact()?;
    ...
    println!(
        "\n{} {}",
        "🎉".bright_green(),
        "Configuration complete!".bright_cyan().bold()
    );
    ...
    let save_config = Confirm::new()
        .with_prompt(format!(
            "{} {}",
            "💾".bright_yellow(),
            "Save this configuration to .ripconfig.toml?"
                .bright_cyan()
                .bold()
        ))
        .default(true)
...
fn print_config_summary(config: &Config) {
    ...
    println!(
        ...
        config.repository_path.display().to_string().bright_white()
    );
    println!(
        "  {} {}",
        "Environment file:".bright_cyan(),
        config.env_filename.bright_white()
    );
    println!(
        "  {} {} items",
        "Selected keys:".bright_cyan(),
        config.env_keys.len().to_string().bright_white()
    );
    println!(
        "  {} {} types",
        "File extensions:".bright_cyan(),
        config.file_extensions.len().to_string().bright_white()
    );
    println!(
        "  {} {} items",
        "Ignore patterns:".bright_cyan(),
        config.ignore_patterns.len().to_string().bright_white()
    );
    println!(
        "  {} {}",
